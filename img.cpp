...
FREE_IMAGE_TYPE free_image_type(ImageType::T destElementType)
{
	switch (destElementType)
	{
	case ImageType::int8bpp: return FIT_UINT8;
	case ImageType::int16bpp: return FIT_UINT16;
	case ImageType::float32bpp: return FIT_FLOAT;
	}
	return FIT_UNKNOWN;
}
...
{
	bmp_handle bmp( FreeImage_AllocateT(free_image_type(destElementType), width, height, bits_per_pixel(destElementType, destChannels)) );
	if (!bmp.valid()) throwx(img_error("Unsupported image type or format (FreeImage)"));
...
bmp_handle save_bmp(stdx::data_range_param<const char> imageData, ImageType::T destElementType, unsigned destChannels, int flags = 0)
{
	FreeImageIO
